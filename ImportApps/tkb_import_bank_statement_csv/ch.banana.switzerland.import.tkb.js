...
function TKBFormat4() {
   ...
   this.colCount = 7; // lines end with separator, is counted as a column (always empty).
   ...
   this.match = function(transactions) {
      ...
      for (i = 0; i < transactions.length; i++) {
        ...
        if (transaction.length === this.colCount)
         ...
         if (formatMatched && transaction[this.colTrNumber] && transaction[this.colTrNumber].match(/[0-9]/))
            formatMatched = true;
         else
            formatMatched = false;
   ...
   this.mapTransaction = function(element) {
      ...
      mappedLine.push(element[this.colTrNumber]);
